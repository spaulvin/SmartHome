...
Action act1 = {pin: D1, topic: "gate/ACTION"};
Action act2 = {pin: D2, topic: "gate/ACTION"};
const int actionsCount = 2;
Action actions[actionsCount] {act1, act2};
...
void mqttCallbackHandler(char *topic, byte *payload, unsigned int length)
{
  ...
  if (strcmp(payloadStr, "open") == 0) {
    ...
    digitalWrite(actions[1].pin, LOW);
    delay(500);
    digitalWrite( actions[1].pin, HIGH);
  }
}
